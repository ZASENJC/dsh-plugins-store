...
describe('ranking page UI', () => {
  ...
  it('renders rankings as a list with inline 24-hour growth and a sparkline', () => {
    ...
    expect(rowSource).toContain("data-field={mode === 'stars24h' ? 'star-change' : 'metric-value'}")
    expect(rowSource).toContain("'ranking.collecting'")
  ...
  it('keeps ranking mode and page origin in URL-backed detail navigation', () => {
    ...
    expect(pageSource).toContain("const rankingModeIds = new Set(['stars24h', 'stars', 'newest', 'updated'])")
  })
})
