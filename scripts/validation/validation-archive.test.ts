...
describe('validation archive merge', () => {
  it('records passed and failed outcomes without changing source exclusion disposition', () => {
    ...
    expect(result.archive.records[1].validation).toMatchObject({ status: 'inconclusive', disposition: 'manual_review', errorCode: 'PLUGIN_LOAD_FAILED' })
    expect(result.manualReview).toEqual([2, 3])
  })
})
