...
`;function we(){let e="dsh-plugins-store-styles";if(document.getElementById(e)!==null)return()=>{};let a=document.createElement("style");return a.id=e,a.textContent=rt,document.head.append(a),()=>a.remove()}var it=["slots","locale","sessions","workspaces"];function ot(e){let s=new j,a=new P,r=e.locale.bind(R);e.effect(()=>e.locale.register(R,{zh:ve,en:xe}),"plugin-store: dictionaries"),e.effect(()=>we(),"plugin-store: styles"),e.on("command/executed",(n,o,l)=>{o==="store"&&l.kind==="success"&&a.open()}),e.slots.inject("shell.overlay",()=>e.slots.register({name:"shell.overlay",id:"plugin-store-dialog",order:40,locale:R,inject:()=>({catalogStore:s,dialogController:a,sessions:e.sessions,workspaces:e.workspaces})},fe)),e.slots.inject("settings.plugins.tab",()=>e.slots.register({name:"settings.plugins.tab",id:"plugin-store",order:20,label:()=>r("settings.tab"),locale:R,inject:()=>({catalogStore:s,sessions:e.sessions,workspaces:e.workspaces})},he));let i=be();i!==null&&a.openInstall(i)}
